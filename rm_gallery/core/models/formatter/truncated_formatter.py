...
from .base_formatter import BaseFormatter, ChatMessage


class TruncatedFormatterBase(BaseFormatter, ABC):
    ...
    async def _format(self, msgs: list[ChatMessage]) -> list[dict[str, Any]]:
    ...
    async def _truncate(self, msgs: list[ChatMessage]) -> list[ChatMessage]:
    ...
    async def _group_messages(
        ...
        Tuple[Literal["tool_sequence", "agent_message"], list[ChatMessage]],
        None,
    ]:
