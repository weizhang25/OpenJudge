...
TRAJECTORY_COMPREHENSIVE_PROMPT_ZH = textwrap.dedent(
    """你是一位专业的评估专家，负责评估智能体轨迹中每个工具调用步骤对问题解决的贡献度。你需要为轨迹中的每个工具调用步骤提供独立且一致的评估，考虑它们各自对问题解决的贡献度以及相对重要性。
...
1. **贡献度评分 (contribution_score)** - 评估此步骤对解决用户问题的整体贡献程度
2. **相关性评分 (relevance_score)** - 评估此步骤与用户查询的相关程度
3. **准确性评分 (accuracy_score)** - 评估此步骤获取或处理信息的准确程度
4. **效率评分 (efficiency_score)** - 评估此步骤的执行效率和必要性
</评估步骤>
...
TRAJECTORY_COMPREHENSIVE_PROMPT_EN = textwrap.dedent(
    """You are a professional evaluation expert responsible for assessing the contribution of each tool call step in an agent trajectory. You need to provide independent and consistent evaluation for each tool call step in the trajectory, considering their respective contributions and relative importance.
...
1. **Contribution Score (contribution_score)** - Evaluate how much this step contributes to solving the user's problem
2. **Relevance Score (relevance_score)** - Evaluate how relevant this step is to the user query
3. **Accuracy Score (accuracy_score)** - Evaluate the accuracy of information obtained or processed in this step
4. **Efficiency Score (efficiency_score)** - Evaluate the efficiency and necessity of this step
</Steps>
